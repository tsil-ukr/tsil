...
namespace tsil::tk {
  CompilerResult Scope::compileBody(const std::vector<ast::ASTValue*>& body) {
    for (const auto& childAstValue : body) {
      ...
      } else if (childAstValue->kind == tsil::ast::KindVariationNode) {
        const auto variationNode = childAstValue->data.VariationNode;
        if (this->hasLocalSubject(variationNode->name)) {
          const auto subject = this->getLocalSubject(variationNode->name);
          if (subject.kind == SubjectKindType) {
            const auto type = subject.data.type;
            if (type->type == TypeTypeVariationInstance) {
              if (type->variationInstanceFields.empty()) {
                int paramIndex = 0;
                for (const auto& variationParamAstValue :
                     variationNode->params) {
                  const auto paramTypeResult = this->bakeType(
                      variationParamAstValue->data.ParamNode->type);
                  ...
                  if (paramTypeResult.type->type == TypeTypeStructureInstance &&
                    ...
                    return {CompilerError::fromASTValue(
                        variationParamAstValue->data.ParamNode->type,
                        "Виявлено неповний тип")};
                  }
                  if (paramTypeResult.type->type == TypeTypeVariationInstance) {
                    return {CompilerError::fromASTValue(
                        variationParamAstValue->data.ParamNode->type,
                        "Виявлено вкладену варіацію")};
                  }
                  const auto field = TypeVariationField{
                      .index = paramIndex,
                      .type = paramTypeResult.type,
                      .name = variationParamAstValue->data.ParamNode->id,
                  };
                  type->variationInstanceFields[variationParamAstValue->data
                                                    .ParamNode->id] = field;
                  paramIndex++;
                }
              } else {
          ...
          type->name = variationNode->name;
          int paramIndex = 0;
          if (variationNode->params.empty()) {
            if (variationNode->type) {
              const auto variationTypeResult =
                  this->bakeType(variationNode->type);
              if (!variationTypeResult.type) {
                return {CompilerError::fromASTValue(variationNode->type,
            ...
            for (const auto& variationParamAstValue : variationNode->params) {
              const auto paramTypeResult =
                  this->bakeType(variationParamAstValue->data.ParamNode->type);
              if (!paramTypeResult.type) {
                return {CompilerError::fromASTValue(
                    variationParamAstValue->data.ParamNode->type,
                    paramTypeResult.error)};
