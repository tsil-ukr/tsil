...
extern ціле __біб__перекодувати_текст_в_ю8(кд* значення, ю8* вихід);
...
extern void __біб__вивести(кд значення) {
  fwrite(значення.дані, значення.розмір, 1, stdout);
}

extern void __біб__надрукувати(кд значення) {
  fwrite(значення.дані, значення.розмір, 1, stdout);
  char newline = 10;
...
extern ціле __біб__шлях_з_тексту(кд* значення, Шлях* вихід) {
  ю8 значення_ю8;
...
extern ціле __біб__доповнити_шлях_текстом(Шлях* шлях, кд* доповнення) {
  ю8 значення_ю8;
...
static void __біб__видалити_рекурсивно(char* шлях) {
  DIR* директорія = opendir(шлях);
  if (директорія) {
    struct dirent* запис;
    while ((запис = readdir(директорія)) != NULL) {
      if (запис->d_name[0] == '.' &&
          (запис->d_name[1] == '\0' ||
           (запис->d_name[1] == '.' && запис->d_name[2] == '\0'))) {
        continue;
      }
      природне довжина_шляху = strlen(шлях);
      природне довжина_імені = strlen(запис->d_name);
      char* новий_шлях = (char*)malloc(довжина_шляху + довжина_імені + 2);
      if (!новий_шлях) {
        closedir(директорія);
        return;
      }
      memcpy(новий_шлях, шлях, довжина_шляху);
      новий_шлях[довжина_шляху] = '/';
      memcpy(новий_шлях + довжина_шляху + 1, запис->d_name, довжина_імені);
      новий_шлях[довжина_шляху + довжина_імені + 1] = '\0';

      __біб__видалити_рекурсивно(новий_шлях);
      free(новий_шлях);
    }
    closedir(директорія);
    rmdir(шлях);
...
extern void __біб__видалити_за_шляхом(Шлях* шлях) {
  char* тимчасовий_буфер = (char*)malloc(шлях->розмір + 1);
  if (!тимчасовий_буфер) {
    return;
  }
  ...
  __біб__видалити_рекурсивно(тимчасовий_буфер);
...
extern ціле __біб__виконати_системну_команду(природне розмір, п8* дані) {
  char* команда = malloc(розмір + 1);
