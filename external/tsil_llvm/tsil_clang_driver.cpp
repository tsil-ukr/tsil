...
extern int cc1_main(ArrayRef<const char*> Argv,
                    const char* Argv0,
                    void* MainAddr);
...
static void getCLEnvVarOptions(std::string& EnvValue,
...
template <class T>
static T checkEnvVar(const char* EnvOptSet,
                     const char* EnvOptFile,
                     std::string& OptFile) {
...
extern "C" int tsil_clang_main(char* path,
  ...
  ToolContext.Path = path;
  ToolContext.NeedsPrependArg = prependArg != nullptr;
  ToolContext.PrependArg = prependArg;
  std::string fullPath;
