...
namespace tsil::parser {
  ...
  std::string АСДЗначенняВString(АСДЗначення* асд_значення, size_t глибина) {
    ...
    if (асд_значення->вид == АСДВидЦіль) {
      const auto data = static_cast<АСДДаніЦіль*>(асд_значення->дані);
      std::string r = "Ціль(";
    ...
    if (асд_значення->вид == АСДВидСекція) {
      const auto data = static_cast<АСДДаніСекція*>(асд_значення->дані);
